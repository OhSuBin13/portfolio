...
from portfolio_app.services import growth as growth_service
...
def create_today_snapshot(db: Db, payload: TodaySnapshotRequest | None = None) -> PortfolioSnapshot:
    ...
    try:
        return growth_service.create_or_refresh_today_snapshot(db, source=snapshot_request.source)
...
def get_snapshots(
    ...
    return growth_service.list_snapshots(db, from_date=from_date, to_date=to_date)
...
def get_growth_history(
    ...
    try:
        return growth_service.build_growth_history(
            db,
            period=period,
